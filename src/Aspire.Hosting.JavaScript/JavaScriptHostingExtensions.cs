        ...
        var resourceBuilder = builder.AddResource(resource)
            ...
            .PublishAsDockerFile(c =>
            {
                ...
                c.WithDockerfileBuilder(resource.WorkingDirectory, dockerfileContext =>
                {
                    ...
                    // Emit a .dockerignore alongside the user's source if one isn't already there. The
                    // runtime stage below copies source directly from the build context (`COPY . .`) so
                    // that the build stage's node_modules (which includes devDependencies) never reach
                    // the runtime image. Without an ignore file the user's local node_modules, .git,
                    // etc. would leak into the build context and into the image.
                    // Matches the recommendation at https://bun.com/guides/ecosystem/docker.
                    var dockerignorePath = Path.Combine(resource.WorkingDirectory, ".dockerignore");
                    if (!File.Exists(dockerignorePath))
                    {
                        File.WriteAllText(dockerignorePath, """
                            # Generated by Aspire. Customize as needed; this file will not be overwritten.
                            node_modules
                            .git
                            .gitignore
                            .DS_Store
                    ...
                    if (emitProdDepsStage)
                    {
                        ...
                        //
                        // A matching .dockerignore is emitted alongside the user's source to ensure local
                        // build artifacts (node_modules, .git, .aspire, etc.) do not leak into the image
                        // via the COPY . . below.
                        runtimeBuilder
